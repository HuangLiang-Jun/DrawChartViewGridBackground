...
open class YAxisRenderer: AxisRendererBase
{
    ...
    // 斑馬紋
    func drawZebra(context: CGContext, positions: [CGPoint]) {
        ...
        {
            let couple = [positions[i], positions[i + 1]]
            couples.append(couple)
        }
        
        // 填充顏色
        // stride: https://goo.gl/LQqrvp
        for j in stride(from: 0, to: couples.count, by: 1) {
            let pots = couples[j]
            let end = pots.first?.y ?? 0.0
            let start = pots.last?.y ?? 0.0
            // 填充的範圍
            ...
            // 填充的顏色，Charts Utils下 hex 轉 UIColor
            let color = ChartColorTemplates.colorFromString("#0072BC")
    ...
    func drawForRange(context: CGContext, positions: [CGPoint]) {
        ...
        let ranges: [[String: Any]] = [["start": getYAxisStandardPosition(from: 0),
                                        "end": getYAxisStandardPosition(from: 20),
                                        "color": UIColor(red:0.84, green:0.93, blue:0.96, alpha:1.0)],
                                       ["start": getYAxisStandardPosition(from: 20),
                                        "end": getYAxisStandardPosition(from: 60),
                                        "color": UIColor(red:0.89, green:0.78, blue:1.00, alpha:1.0)],
                                       ["start": getYAxisStandardPosition(from: 60),
                                        "end": getYAxisStandardPosition(from: 100),
                                        "color": UIColor(red:1.00, green:1.00, blue:0.75, alpha:1.0)]]
